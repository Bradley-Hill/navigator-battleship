...
test("Expect the computer to continue attacking adjacent cells after a confirmed hit until the ship is sunk", () => {
  const humanPlayer = createPlayer(true);
  const compPlayer = createPlayer(false);
  const ship = humanPlayer.gameboard.createShips(1, 1, 3, "horizontal");
  ...
  while (!hit) {
    ...
    const hitCells = humanPlayer.gameboard.getHitCells();
    hit = hitCells.some(
      ([x, y]) => humanPlayer.gameboard.grid[x][y].ship === ship
    );
  }

  const initialHitCell = humanPlayer.gameboard.getHitCells()[0];

  let attempts = 0;
  const maxAttempts = 100; // Adjust this value as needed
  while (ship && !ship.sunk && attempts < maxAttempts) {
    compPlayer.makeComputerMove(humanPlayer);
    attempts++;
  }

  if (attempts === maxAttempts) {
    throw new Error("Failed to sink the ship after maximum attempts");
  }
  ...
  // Check that the computer has hit adjacent cells
  const finalHitCells = humanPlayer.gameboard.getHitCells();
  finalHitCells.forEach(([x, y]) => {
    const isAdjacent =
      Math.abs(x - initialHitCell[0]) <= 1 &&
      Math.abs(y - initialHitCell[1]) <= 1;
    expect(isAdjacent).toBe(true);
  });
});
